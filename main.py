...
async def handle_chat_join_request(event: types.ChatJoinRequest, bot: Bot):   
    ...
    chat_invite_link1 = await bot.create_chat_invite_link(-1002533324913)
    chat_invite_link2 = await bot.create_chat_invite_link(-1002831908480)
    await bot.send_photo(event.from_user.id, photo=types.FSInputFile("image.png"), caption="𝗔𝘀𝘀𝗮𝗹𝗼𝗺𝘂 𝗮𝗹𝗮𝘆𝗸𝘂𝗺😊\n\n𝗦𝗶𝘇 𝗩𝗶𝗽 𝗸𝗮𝗻𝗮𝗹 𝘃𝗮 𝗦𝗵𝗼𝗸𝗵 𝗮𝗸𝗲𝗻𝗶𝗻𝗴 𝟯𝟬𝟬𝟬$ 𝗹𝗶𝗸 𝗸𝘂𝗿𝘀𝗶𝗻𝗶 𝘆𝘂𝘁𝗶𝗯 𝗼𝗹𝗱𝗶𝗻𝗴𝗶𝘇😎\n\n𝗨𝗹𝗮𝗿𝗻𝗶 𝗾𝗼𝗹𝗴𝗮 𝗸𝗶𝗿𝗶𝘁𝗶𝘀𝗵 𝘂𝗰𝗵𝘂𝗻 𝗽𝗮𝘀𝗱𝗮𝗴𝗶 𝗧𝘂𝗴𝗺𝗮𝗹𝗮𝗿𝗱𝗮𝗻 𝗳𝗼𝘆𝗱𝗮𝗹𝗮𝗻𝗶𝗻𝗴👇", reply_markup=(send_buttons(chat_invite_link1, chat_invite_link2)))
    # await event.approve() # bu kod foydalanuvchi zayavka tashalasa avtomatik qabul qiladi
